"""Модуль для расчета предматчевой статистики команд.

Модуль предоставляет функционал для вычисления статистики голов и результатов
команд на основе исторических данных о проведенных матчах. Статистика
рассчитывается в разрезе всех матчей, домашних и выездных игр, а также
в зависимости от исхода матча (победа, ничья, поражение).

Статистика используется для прогнозирования результатов матчей и расчета ставок.
"""
...
def _update_stats(
    ...
    Функция обновляет все поля статистики на основе результата одного матча.
    ...
    :param stats: Объект статистики, который необходимо обновить.
    ...
    :return: None. Функция модифицирует переданный объект stats in-place.
    """
    ...
    # Используем Decimal для точного сложения средних значений
    ...
    if matches_count != 0:
        ...
        # Процент ничьих вычисляется как остаток от 100%
...
def _update_team_stat(
    ...
    Функция обновляет статистику команды сразу в трех разрезах:
    ...
    :return: None. Функция модифицирует переданный объект team_stats in-place.
    """
    # Определяем количество забитых и пропущенных голов на основе флага is_home
...
def calculate_league_prematch_stats(
    ...
    Функция выполняет последовательный проход по списку матчей
    ...
    :return: Словарь, где ключом является идентификатор матча, а значением -
        объект MatchStatistics с предматчевой статистикой для обеих команд.
    """
